...
| POST | `/api/integrations` | Railway service 연결 — `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` + `OTEL_EXPORTER_OTLP_PROTOCOL=http/json` 항상 주입; `verum_api_key` 파라미터 제공 시 `OTEL_EXPORTER_OTLP_HEADERS=Authorization=Bearer <key>` 추가 주입; `inject_node_options=true`면 `NODE_OPTIONS` 추가. `railway_token` AES-256-GCM 암호화 저장. `integrations` 행 생성 |
...
| `apps/dashboard/src/app/api/integrations/route.ts` | GET: 사용자 Railway integration 목록 (token 제외). POST: Railway service 연결 — `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` + `OTEL_EXPORTER_OTLP_PROTOCOL=http/json` 항상 주입; `verum_api_key` 제공 시 `OTEL_EXPORTER_OTLP_HEADERS` 추가; `inject_node_options=true`면 `NODE_OPTIONS` 추가; token AES-256-GCM 암호화 저장 |
...
- ✅ PR #115: 코드 품질 수정 — `harvest/repository.py`·`analyze/python_analyzer.py` `zip(strict=True)` 추가; `harvest/repository.py` `vector_search()` SQL 파라미터화(string concat 제거); 14개 TypeScript 라우트 auth 패턴 `getAuthUserId()` 통일; `worker/listener.py` `logger.warning` → `logger.exception()` 수정.
- ✅ PR #116: Railway OTLP env var 수정 — `OTEL_EXPORTER_OTLP_ENDPOINT` → `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`; `OTEL_EXPORTER_OTLP_PROTOCOL=http/json` 항상 주입; `verum_api_key` (optional, min 40자) 파라미터 추가 → `OTEL_EXPORTER_OTLP_HEADERS=Authorization=Bearer <key>` 주입. 테스트 23개 추가 (integrations route 전체 커버).
- ✅ PR #131: Zod v3 → v4 마이그레이션 (`zod` ^3.25.76 → ^4.4.3). `z.string().uuid()` v4에서 엄격한 RFC 4122 검증 적용 — 7개 API route 테스트 파일 + 3개 Playwright E2E 테스트 파일의 UUID 픽스쳐를 RFC 4122 준수 형식으로 업데이트. Dashboard Jest 365 → 432, Playwright ~16 → 31.
...
| Dashboard Jest | 49 suites | 432 | 2026-05-05 |
| E2E Playwright | 3 spec | 31 | 2026-05-05 |
...
_Last updated: 2026-05-05 (PR #115: zip(strict=True)/SQL parameterization/getAuthUserId auth pattern/logger.exception fixes; PR #116: OTLP env var rename OTEL_EXPORTER_OTLP_TRACES_ENDPOINT + PROTOCOL always injected + optional HEADERS via verum_api_key; Dashboard Jest suite count updated 39→49; PR #131: Zod v3→v4 migration, UUID fixtures RFC 4122 compliance, Dashboard Jest 365→432, Playwright ~16→31) | Maintained by: Claude at end of each implementation session_
