...
| GET | `/api/v1/activation/[repoId]` | ActivationCard data — INFER/GENERATE/HARVEST summary for a repo (JWT session auth) |
...
**ActivationCard:** Dashboard UI at `GET /api/v1/activation/[repoId]` shows INFER/GENERATE/HARVEST results and surfaces two integration PR options (Phase 0 or Phase 1) before the user installs the SDK.
...
| `apps/dashboard/src/app/repos/[id]/StagesView.tsx` | Loop 8단계 수평 스테퍼 (done/active/pending 시각 상태) + 실행 중 단계 카드(좌측 색상 border + 진행 바) + Quick Stats 3-grid + OBSERVE·EXPERIMENT 섹션 마운트 |
...
| `apps/dashboard/src/components/repo/ActivationCard.tsx` | SDK 설치 전 통합 선택 UI (Phase 0 OTLP / Phase 1 bidirectional 두 모드 버튼). Phase 0·1 각각의 기존 PR 상태를 DB에서 별도 로드 (`existingPrUrl`/`existingBidirectionalPrUrl` props) |
...
_Last updated: 2026-05-01 (PR #102 — INFER 쿼터 체크, GENERATE 이메일 알림, 대시보드 UI 수정; PR #103 — Python/TypeScript SDK Phase 0.5 zero-code-change auto-patch) | Maintained by: Claude at end of each implementation session_
