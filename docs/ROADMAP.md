...
| F-4.12 | Railway platform integration: connect/disconnect UI + AES-256-GCM token encryption + OTLP env var auto-injection (`OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_PROTOCOL`, optional `OTEL_EXPORTER_OTLP_HEADERS`) + optional `NODE_OPTIONS` (`integrations` table, migration 0025) (PR #112, PR #116) | ✅ |
