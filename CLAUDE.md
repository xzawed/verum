...
- **Phase 1.5 (코드 변경 0줄 + Bidirectional)**: `pip install verum` 설치 후 `VERUM_API_URL` + `VERUM_API_KEY` 환경변수만 설정. `verum-auto.pth`가 site-packages에 자동 설치되어 Python 시작 시 OpenAI/Anthropic 클라이언트를 자동 패치. Node.js는 `NODE_OPTIONS="--require @verum/sdk/auto"` 추가. 코드 변경 0줄로 Phase 1과 동일한 auto-instrument 달성. `VERUM_DISABLED=1`로 비활성화. ADR-018 참조.
...
- **PR 머지 후 자발적 회고**: xzawed가 요청하지 않아도 주요 PR 머지 후 짧은 회고를 먼저 꺼낸다. "잘 된 것 / 다음에 바꿀 것" 두 항목만, 길지 않게
- **판단 위임 시 추천을 명시**: xzawed가 "Claude 판단에 맡기겠습니다"라고 하면 조용히 실행하지 말고 "A를 추천합니다. 이유는 X. B를 선택하면 Y가 달라집니다"로 한 문장 먼저 확인받는다
- **예상 밖 수정 시 이유 먼저**: 계획과 다른 방향으로 수정할 때는 "왜"를 먼저 설명하고 선택지를 제시한다. 결론을 먼저 말하면 변명처럼 들린다
